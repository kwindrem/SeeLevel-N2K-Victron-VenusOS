...
// may need to be changed on each system
	property string seeLevelServieName: "com.victronenergy.tank.socketcan_can0_di0_uc855"
//////// SeeLevel -- end add this
	...
	ListView {
		...
		property int tileHeight: Math.ceil(height / Math.max(count, 3))
		width: 134
		interactive: false // static tiles
		...
		Tile {
			...
			anchors.fill: parent
			values: TileText {
// hide column text because tanks will come and go based on connectivity
				text: qsTr("")
				width: parent.width
...
// hide the service for the physical sensor system because it shows values for multiple tanks 
			if (service.name !== seeLevelServieName) // skip N2K SeeLevel dBus object
//////// SeeLevel -- end add this
