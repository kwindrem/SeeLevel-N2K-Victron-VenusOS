...
Innitially, I was told that SeeLevel reports one tank approximately every 1-2 seconds. A complete scan of all tanks takes up to 3-8 seconds. Repeater service timeout is set to 20 seconds. However I discovered tanks can be reported much more rapidly. Too fast actually for my original design, so it's been rewritten to scan the SeeLevel dBus object rapidly to prevent missing an update. I ended up polling every 50 mS and still occationally miss an update!!!
